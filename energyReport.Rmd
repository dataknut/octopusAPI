...
recent_Hourlydt <- elecCons_dt[dv_date >= today - 14 , .(sum_kWh_elec = sum(consumption_kWh)), 
                          keyby = .(Date = dv_date,
                                    hour = lubridate::hour(dv_start))]

ggplot2::ggplot(recent_Hourlydt, aes(x = hour, y = sum_kWh_elec, 
                                     alpha = Date, group = Date)) +
  geom_line() +
  labs(caption = "Most recent 14 days")
```
> Can we [flex](https://twitter.com/tom_rushby/status/1589653290609913856) any of that for £3/kWh?
