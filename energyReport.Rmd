...
Instigated by Uk National gird, implemented (in our case) by Octopus.

#### 15th Nov 2022

17:00 - 18:00 £2.50/kWh

Figure \@ref(fig:firstSavingSessionUsage) shows how we did in the first #SavingsSession compared to the last 8 similar days of the week.

```{r firstSavingSessionUsage, fig.cap="First #SavingsSession - how did we do?"}
...
session_HalfHourlyDT[, legend_lab := paste0("SavingsSession day: ", 
...
> What do we conclude? Our electricity usage was probably slightly higher than 'average' for this time on a Tuesday (so we won't win any points) but it was probably **way lower** than it otherwise would have been judging by the spikes before and after.

For fun - Figure \@ref(fig:firstSavingSessionCI) shows the mean carbon intensity in g CO2(e) per kWh for each half-hour.

```{r firstSavingSessionCI, fig.cap="First #SavingsSession - comparison of carbon intensity"}
...
session_HalfHourlyCIDT <- ngeso_dt_orig[dv_date == lubridate::date(sessionDateStart),  #  the session day
                          ...
                          keyby = .(hms = dv_hms,
                                    wday = lubridate::wday(dv_date, label = TRUE))] # to check
session_HalfHourlyCIDT[, legend_lab := paste0("SavingsSession day: ", 
                                      lubridate::wday(sessionDate, label = TRUE),
                                      " ", sessionDate, " 17:00 - 18:00")]
