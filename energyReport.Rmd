...
today <- lubridate::today()
plotDT <- elecCons_dt[dv_date >= today - 14]
...
We missed this one but Figure \@ref(fig:tenthSavingSessionsUsage) shows how we did in this #SavingSession compared to the last n similar days of the week - had we signed up.
...
today <- lubridate::today()
plotDT <- gasCons_dt[dv_date >= today - 14]
p <- ggplot2::ggplot(plotDT, aes(x = dv_date, y = dv_hms, fill = consumption_kWh)) +
  geom_tile() +
  theme(legend.position = "bottom") +
  scale_fill_viridis_c(name = "Gas import (kWh)") +
