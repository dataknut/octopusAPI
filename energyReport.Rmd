...
addDerivedVariables <- function(dt, source){
  ...
  if(source == "NGESO"){
    dt[, dv_start := lubridate::as_datetime(DATETIME)]
  }
  
  dt[, dv_hh_start_hms := hms::as_hms(dv_start)]
  dt[, dv_hh_start_date := lubridate::as_date(dv_start)]
  dt[, dv_month := lubridate::floor_date(dv_hh_start_date, unit = "months")]
  dt[, dv_year := lubridate::year(dv_hh_start_date)]
  
  dt[, dv_peakPeriod := ifelse(dv_hh_start_hms < as_hms("06:00:00"), "Early morning (00:00 - 06:00)", NA)]
dt[, dv_peakPeriod := ifelse(dv_hh_start_hms >= as_hms("06:00:00") &
                                          dv_hh_start_hms < as_hms("09:00:00"), 
                                        "Morning peak (06:00 - 09:00)", 
                                        dv_peakPeriod)]
dt[, dv_peakPeriod := ifelse(dv_hh_start_hms >= as_hms("09:00:00") &
                                          dv_hh_start_hms < as_hms("16:00:00"), 
                                        "Day time (09:00 - 16:00)", 
                                        dv_peakPeriod)]
dt[, dv_peakPeriod := ifelse(dv_hh_start_hms >= as_hms("16:00:00") &
                                          dv_hh_start_hms < as_hms("20:00:00"), 
                                        "Evening peak (16:00 - 20:00)", 
                                        dv_peakPeriod)]
dt[, dv_peakPeriod := ifelse(dv_hh_start_hms >= as_hms("20:00:00"), 
...
levels <- c("Early morning (00:00 - 06:00)", 
              ...
              "Day time (09:00 - 16:00)", 
              "Evening peak (16:00 - 20:00)", 
              "Late evening (20:00 - 00:00)") # set order nicely for plots & tables
# must match labels above

 dt[, dv_peakPeriod := factor(dv_peakPeriod, levels)] # set order for plots etc
  
  dt[, dv_weekend := ifelse(lubridate::wday(dv_hh_start_date, label = TRUE) == "Sat",
                            "Saturday",
                            "Weekday")]
  dt[, dv_weekend := ifelse(lubridate::wday(dv_hh_start_date, label = TRUE) == "Sun",
                            "Sunday",
                            dv_weekend)]
  return(dt)
}
...
### PV generation (needs fixing to use PV export data)
...
Figure \@ref(fig:eleventhSavingSessionsUsage) shows how we would have done in this #SavingSession compared to the last n similar days of the week.

```{r eleventhSavingSessionsUsage, fig.cap="Eleventh #SavingSessions- how would we have done?"}
if(as.Date(startDateTime) < Sys.Date()){ # octopus data only available for day before
  data <- TRUE
  res <- make_kwhComparisonPlot(elecCons_dt, 
                                startDateTime, endDateTime)
...
Figure \@ref(fig:twelthSavingSessionsUsage) shows how did in this #SavingSession compared to the last n similar days of the week.

```{r twelthSavingSessionsUsage, fig.cap="Twelth #SavingSessions- how did we do?"}
if(as.Date(startDateTime) < Sys.Date()){ # octopus data only available for day before
  data <- TRUE
  res <- make_kwhComparisonPlot(elecCons_dt, 
                                startDateTime, endDateTime)
...
Figure \@ref(fig:thirteenthSavingSessionsUsage) shows how did in this #SavingSession compared to the last n similar days of the week.

```{r thirteenthSavingSessionsUsage, fig.cap="Thirteenth #SavingSessions- how did we do?"}
if(as.Date(startDateTime) < Sys.Date()){ # octopus data only available for day before
  data <- TRUE
  res <- make_kwhComparisonPlot(elecCons_dt, 
                                startDateTime, endDateTime)
...
Figure \@ref(fig:plotGasImport) shows half-hourly gas import ('consumption') for the current year. The power cuts are even easier to see here. 

Gas boiler services:

 * June 2022 - interestingly the pattern after the gas boiler was serviced in June 2022 is more varied. _What did he change_?
 * 15th June 2023
