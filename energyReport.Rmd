...
Figure \@ref(fig:firstSavingSessionsUsage) shows how we did in the first #SavingSessions compared to the last n similar days of the week.
...
xmin <- hms::as_hms(lubridate::as_datetime(sessionDateStart))
xmax <- hms::as_hms(lubridate::as_datetime(sessionDateEnd)) + 30*60 # to allow for the start time
...
wt[, kwh_diff := `SavingSessions day: Tue 2022-11-15 17:00 - 18:00` - `Mean of the previous 10 Tuesdays`]
wt[, pc_diff := 100*(kwh_diff/`Mean of the previous 10 Tuesdays`)]

makeFlexTable(wt[, hms:= as.factor(hms)], cap = "kWh comparisons (hms = half hour period start", digits = 3)
```
...
For fun - Figure \@ref(fig:firstSavingSessionsCI) shows the mean carbon intensity in g CO2(e) per kWh for each half-hour.

```{r firstSavingSessionsCI, fig.cap="First #SavingSessions - comparison of carbon intensity"}
...
Figure \@ref(fig:thirdSavingSessionsUsage) shows how we did in the first #SavingSessions compared to the last 8 similar days of the week.

```{r thirdSavingSessionsUsage, fig.cap="Third #SavingSessions - how did we do?"}
sessionDateStart <- "2022-11-30 17:30:00" # the half-hour it starts
sessionDateEnd <- "2022-11-30 18:00:00" # the half-hour it ends (makes data selection easier)
...
wt[, kwh_diff := `SavingSessions day: Wed 2022-11-30 17:30 - 18:30` - `Mean of the previous 8 Wednesdays`]
wt[, pc_diff := 100*(kwh_diff/`Mean of the previous 8 Wednesdays`)]

makeFlexTable(wt[, hms:= as.factor(hms)], cap = "kWh comparisons (hms = half hour period start", digits = 3)
```

> What do we conclude? 
> Cost centre #3 rushed home to get a toastie & his laundry in before 17:30 and then went to rugby training
> Cost centre #4 just went to the gym
> Our dinner guest was happy to delay cooking & eating slightly

For fun - Figure \@ref(fig:thirdSavingSessionsCI) shows the mean carbon intensity in g CO2(e) per kWh for each half-hour.

```{r thirdSavingSessionsCI, fig.cap="First #SavingSessions - comparison of carbon intensity"}
...
```{r plotTotalElecImportDaily, fig.cap="Mean half-hourly electricity import per day (kWh, current year)"}
...
These use [the latest Octopus flexible tariff from Oct 2022](https://octopus.energy/blog/energy-price-cap-oct-2022/#flexibleoct22rates) which are protected ny the government's Energy Price Guarantee:

* electricity: £`r round(prices$elec_kWh,2)` / kWh and £`r round(prices$elec_sc,2)` / day (standing charge) 
* gas: £`r round(prices$gas_kWh,2)` / kWh and £`r round(prices$gas_sc,2)` / day (standing charge)
