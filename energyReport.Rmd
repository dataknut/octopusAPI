...
make_kwhComparisonPlot <- function(dt, # half hourly smart meter data
  ...
  baseline_meanHalfHourlyDT <- dt[dv_date != sessionDate & # not the session day
                               #lubridate::wday(dv_date, label = TRUE) == sessionDay & # same day of the week
                               dv_date > sessionDate - (timeLag + 1) & # n days before
                               dv_date < sessionDate, # not after the session
    ...
    res$p <- ggplot2::ggplot(plotDT, aes(x = adjusted_hms, y = mean_kWh_elec, 
      ...
      labs(x = "Time of day",
           ...
           caption = paste0("Data: @OctopusEnergy\nPlot: @dataknut\nPoints centered in half-hours for clarity\nComparison = ", res$nCompDays," previous days (",
                            pd(min(baseline_meanHalfHourlyDT$minDate)), " - ",
                                pd(max(baseline_meanHalfHourlyDT$maxDate)), ")"
                            )
      )
...
Figure \@ref(fig:firstSavingSessionsUsage) shows how we did in the first #SavingSessions **compared to the last 10 days**.
 ...
 * As we've seen on previous sessions, our (cooking) 'peak' is usually c. 19:00 so these sessions are a bit early for us - they don't coincide with peak! Our 16:00 pick-up will be the heating/gas boiler pump.
